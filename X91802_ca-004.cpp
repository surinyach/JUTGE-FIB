...
using namespace std; 
    ...
    hi ha cap estudiant amb nota, totes dues components valen -1 */
 {
    ...
    for (int i = 0; i < v.size(); ++i) {
        ...
        if (v[i].te_nota() and not primer) {
            if (v[i].consultar_nota() > v[p.first].consultar_nota() and v[i].consultar_DNI() < v[p.first].consultar_DNI()) {
                p.first = i;
            }
            if (v[i].consultar_nota() < v[p.second].consultar_nota() and v[i].consultar_DNI() < v[p.second].consultar_DNI()) {
                p.second = i; 
            }
        }
    }
